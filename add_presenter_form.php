...
<body>
    <div class="container">
        <div class="box form-box">
            <header>Add Presenter</header>
            <form action="add_presenter.php?event_id=<?php echo $_GET['event_id'] ?>" method="post">

                <div class="field input:">
                    <label for="presenter">Add a presenter:</label>
                    <select name="presenter" id="presenter">
                        <?php while ($row = mysqli_fetch_assoc($presenter_query)) {
                        ?>
                            <option value="<?php echo $row["Presenter_id"] ?>"><?php echo $row['First_name'] ?> <?php echo $row['Last_name'] ?></option>;
                        <?php } ?>
                    </select>
                </div>
